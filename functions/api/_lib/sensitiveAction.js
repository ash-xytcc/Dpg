...
async function verifyPassword(password, storedHash) {
  ...
  let raw;
  try { raw = fromB64(storedHash); } catch { return false; }
...
async function verifyMfaIfEnabled({ db, env, userId, code }) {
  let row = null;
  try {
    row = await db.prepare('SELECT totp_secret_encrypted, mfa_enabled FROM user_mfa WHERE user_id=?')
      .bind(userId).first();
  } catch (error) {
  ...
  if (!/^\d{6}$/.test(normalized)) return { ok: false, resp: bad(400, 'MFA_REQUIRED') };

  let encrypted = row.totp_secret_encrypted;
  if (!encrypted) return { ok: false, resp: bad(400, 'MFA_SECRET_MISSING') };
  if (typeof encrypted === 'string') {
    try { encrypted = JSON.parse(encrypted); }
    catch { return { ok: false, resp: bad(400, 'MFA_SECRET_CORRUPT') }; }
  }
  ...
  let secret;
  try { secret = await aesGcmDecrypt(encrypted, encryptionKey); }
  catch { return { ok: false, resp: bad(400, 'MFA_SECRET_DECRYPT_FAILED') }; }

  const valid = await totpVerify(secret, normalized, { window: 1 });
  return valid
    ? { ok: true, required: true }
    : { ok: false, resp: bad(400, 'INVALID_MFA_CODE') };
}
...
export async function requireSensitiveAction({ env, request, password, mfaCode }) {
  ...
  const limitKey = `sensitive:${userId}`;
  ...
  if (Number(attempt?.count) > 12) {
    return { ok: false, resp: bad(429, 'RATE_LIMIT', { retry_after: Math.max(1, Math.ceil((attempt.reset_at - timestamp) / 1000)) }) };
  }

  const account = await db.prepare('SELECT id,email,name,password_hash FROM users WHERE id=? LIMIT 1')
    .bind(userId).first();
  if (!account) return { ok: false, resp: bad(401, 'UNAUTHORIZED') };

  if (!await verifyPassword(password, account.password_hash)) {
    return { ok: false, resp: bad(401, 'INVALID_PASSWORD') };
  }
