...
export async function getOrgLockdownState({ env, orgId }) {
  ...
  const row = await db.prepare('SELECT lockdown_enabled FROM org_emergency_state WHERE org_id=?')
    .bind(orgId).first();
...
export async function getOrgIsolationState({ env, orgId }) {
  ...
  const row = await db.prepare('SELECT isolated, stage FROM emergency_protocol_state WHERE org_id=?')
    .bind(orgId).first();
  return {
...
export async function enforceOrgIsolationAccess({ env, orgId }) {
  ...
  return state.isolated ? { ok: false, resp: bad(403, 'ORG_ISOLATED') } : { ok: true };
}
...
export async function enforceOrgWriteLockdown({ env, orgId }) {
  ...
  return (state.enabled || protocol.stage !== 'normal')
    ? { ok: false, resp: bad(403, 'ORG_LOCKDOWN_ACTIVE') }
    : { ok: true };
}
