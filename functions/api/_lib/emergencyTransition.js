...
export async function transitionEmergency({ env, orgId, userId, from, stage, summary }) {
  ...
  const results = await db.batch([
    db.prepare(`INSERT INTO org_emergency_state
      ...
      SELECT org_id,?,?,?,?,?,?,? FROM emergency_protocol_state
      ...
      ON CONFLICT(org_id) DO UPDATE SET
        ...
        lockdown_cleared_at=excluded.lockdown_cleared_at,
    ...
    db.prepare(`INSERT INTO emergency_reports
      (id,org_id,event_type,severity,status,actor_user_id,summary,details_json,created_at)
      SELECT ?,org_id,?,?, 'recorded',?,?,NULL,? FROM emergency_protocol_state
      WHERE org_id=? AND stage IN (${placeholders})`)
    ...
    db.prepare(`UPDATE emergency_protocol_state SET stage=?,isolated=?,
      ...
      recovered_at=CASE WHEN ?=0 THEN ? ELSE NULL END,
      updated_at=?
      WHERE org_id=? AND stage IN (${placeholders})`)
  ...
  if (Number(results[2]?.meta?.changes) !== 1) {
    return { ok: false, resp: bad(409, 'PROTOCOL_STAGE_CHANGED') };
  }

  return {
    ok: true,
    protocol: { stage, isolated: !!isolated, updatedAt: timestamp },
    state: { orgId, enabled: !!locked, updatedAt: timestamp },
  };
}
