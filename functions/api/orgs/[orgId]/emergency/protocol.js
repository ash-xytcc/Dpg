...
export async function onRequestGet({ env, request, params }) {
  ...
  const row = await state.db.prepare('SELECT stage,isolated,isolated_at,updated_at FROM emergency_protocol_state WHERE org_id=?')
    .bind(orgId).first();
  const lockdown = await state.db.prepare('SELECT lockdown_enabled FROM org_emergency_state WHERE org_id=?')
    .bind(orgId).first();
  const stage = String(row?.stage || (lockdown?.lockdown_enabled ? 'lockdown' : 'normal'));
  const owner = String(gate.role || '').toLowerCase() === 'owner';
  let preview = null;
  ...
  return ok({
    protocol: { stage, isolated: !!row?.isolated, isolatedAt: row?.isolated_at || null, updatedAt: row?.updated_at || null },
    permissions: { canLockdown: owner || String(gate.role).toLowerCase() === 'admin', canDestroy: owner },
    preview,
