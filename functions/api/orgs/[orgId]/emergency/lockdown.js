...
export async function onRequestPost({ env, request, params }) {
  ...
  const result = await transitionEmergency({
    env,
    orgId,
    userId: gate.user.sub,
    from: ['normal','lockdown'],
    stage: enabled ? 'lockdown' : 'normal',
    summary: enabled ? 'Recoverable organization lockdown enabled' : 'Organization lockdown cleared',
  });
