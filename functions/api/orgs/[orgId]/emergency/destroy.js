...
export async function onRequestPost({ env, request, params }) {
  ...
  const protocol = await state.db.prepare('SELECT stage,isolated FROM emergency_protocol_state WHERE org_id=?')
    .bind(orgId).first();
  ...
  if (body?.acknowledgeHistoricalLimit !== true) return bad(400, 'HISTORICAL_ERASURE_ACK_REQUIRED');

  const transition = await transitionEmergency({
    env,
    orgId,
    userId: fresh.user.sub,
    from: ['prepared','destroying'],
    stage: 'destroying',
    summary: 'Permanent organization destruction started',
  });
  if (!transition.ok) return transition.resp;

  let result;
  try { result = await destroyOrgData({ env, db: fresh.db, orgId }); }
  catch (error) { return bad(500, error?.message || 'ORG_DESTROY_FAILED'); }
