...
export async function onRequestPost({ env, request }) {
  ...
  if (!result.ok) return bad(409, 'SOLE_OWNED_ORGS_REMAIN', { orgs: result.blockers });
  ...
  const response = ok({
    ...
    erasure: {
      ...
      limitation: 'Provider-managed historical backups may persist outside the live DPG application.',
    },
  });
